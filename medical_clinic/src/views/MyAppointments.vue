      ...
      <table class="fl-table">
        ...
        <tbody v-for="(elem, index) in my_appointments" :key=elem>
          <tr v-if="index >= min && index < max">
            ...
            <td><select type='text' :id="'EDIT_REFFERAL_' + elem.id"  :value="elem.price !== '0' ? 'Nu' : 'Da'" disabled @click="chooseRefferal($event, elem)"> 
                  <option value='Da'>Da</option>
                  <option value='Nu'>Nu</option>
                </select></td>
            <td :id="'EDIT_PRICE_' + elem.id">{{elem.price}} lei</td>
            <td><button  type="submit" :id="'EDIT_' + elem.id" class="cancel_appointment" v-if="elem.date.setHours(0,0,0,0) >= new Date().setHours(0,0,0,0)" @click="edit_app(elem, index)">Editeaza</button>
            <span v-else>Nu se poate edita</span></td>
            <td><button type="submit" :id="'DELETE_' + elem.id" class="delete_appointment" v-if="elem.date.setHours(0,0,0,0) >= new Date().setHours(0,0,0,0)" @click="deleteAppointment(elem.id)">Anuleaza</button>
            <span v-else>Nu se poate anula</span></td>
          </tr>
        </tbody>
      </table>
    </div>
...
export default {
  ...
  methods: {
    ...
    deleteAppointment(id_app) {
      let idDelete = id_app;
      console.log('Vreau sa sterg programarea: ' + idDelete);
      ...
      fetch(url + 'deleteApp/' + idDelete, requestParameters)
      .then(res => res.json())
      .then(res => {
        if (res.message === 'Decoding error!' || res.message === "Your token expired!") {
          ...
          this.$store.dispatch("deleteAppointment", idDelete);
          this.my_appointments = this.my_appointments.filter(element => element.id !== idDelete);
          this.getSize();
        }
      })
    },
    ...
    edit_app(elem, index) {
      if(document.getElementById('EDIT_' + elem.id).innerHTML === 'Editeaza') {
        document.getElementById('EDIT_REFFERAL_' + elem.id).disabled = false;
        document.getElementById('EDIT_' + elem.id).innerHTML = 'Salveaza';
        document.getElementById('EDIT_' + elem.id).style.color = 'green';
      } else {
        if( document.getElementById('EDIT_PRICE_' + elem.id).innerHTML.split(" ")[0] === elem.price)
          console.log('Nu au fost date schimbate!');
        else {
          let requestParameters = utils.globalRequestParameters;
          let token = window.localStorage.getItem('token');
          requestParameters.headers.Authorization = 'Bearer ' + token;
          ...
          let data = {};
          data.id = elem.id;
          data.doctorName = elem.doctorName;
          data.doctorSpecialization = elem.doctorSpecialization;
          data.date = `${elem.date.getDate()}-${elem.date.getMonth()+1}-${elem.date.getFullYear()}`;
          data.hour = elem.hour;
          data.pacientName = elem.pacientName;
          data.pacientYearsOld = elem.pacientYearsOld;
          data.price = document.getElementById('EDIT_PRICE_' + elem.id).innerHTML.split(" ")[0];

          requestParameters.body = JSON.stringify(data);

          console.log("Vreau sa editez programarea: " + data.id);

          fetch(utils.url + 'edit_appointment/' + data.id, requestParameters)
          .then( res => res.json())
          .then( res => {
            if (res.message === 'Decoding error!' || res.message === "Your token expired!")
              ...
              console.log(res.message);
              this.my_appointments[index].price = document.getElementById('EDIT_PRICE_' + elem.id).innerHTML.split(" ")[0];
              this.backup_appointments[index].price = document.getElementById('EDIT_PRICE_' + elem.id).innerHTML.split(" ")[0];
              document.getElementById('EDIT_REFFERAL_' + elem.id).disabled = true;
              document.getElementById('EDIT_' + elem.id).innerHTML = 'Editeaza';
              document.getElementById('EDIT_' + elem.id).style.color = 'black';
    ...
    chooseRefferal(event, elem) {
      if (event.target.value === 'Da') {
        document.getElementById('EDIT_PRICE_' + elem.id).innerHTML = '0 lei';
      } else {
        if(elem.specialization === "Alergologie"){
          document.getElementById('EDIT_PRICE_' + elem.id).innerHTML = '300 lei';
        } else if (elem.specialization === "Cardiologie"){
          document.getElementById('EDIT_PRICE_' + elem.id).innerHTML = '250 lei';
        } else {
          document.getElementById('EDIT_PRICE_' + elem.id).innerHTML = '200 lei';
        }
      }
    },
  }
